...
{
	GPIO_voidSetPortPinValue(PORT_A , PIN1 , 0) ;   //// RW LOW == Selects write
	GPIO_voidSetPortValue(PORT_B , Byte) ;
	GPIO_voidSetPortPinValue(PORT_A , PIN2 , 1) ;   //// E HIGH == Reset
	_delay_ms( 1 ) ;
	GPIO_voidSetPortPinValue(PORT_A , PIN2 , 0) ;   //// E LOW == Making falling edge
}
...
{
	GPIO_voidSetPortPinValue(PORT_A , PIN0 , 0) ;   //// RS LOW == Selects IR
	send( CMD ) ;
...
{
	GPIO_voidSetPortPinValue(PORT_A , PIN0 , 1) ;   //// RS HIGH == Selects DR
	send( Char ) ;
}
